 ...
 * Modified to use em units instead of pixels for better responsiveness
 */
const DedraLoader = (function () {
  // SVG logo content - paste your SVG here
  let logoSvgContent = `<svg width="15.625em" height="15.625em" viewBox="0 0 250 250" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  const languages = {
    ...
    bn: {
      ...
      texts: [
        ...
        "গেম সম্পদ লোড হচ্ছে",
        "চরিত্র লোড হচ্ছে",
        "নিয়ন্ত্রণগুলি ক্যালিব্রেট করা হচ্ছে",
        "প্রায় সেখানে",
        "লোড সম্পূর্ণ!",
      ],
    },
    ...
    nl: {
      ...
      texts: [
        "Initialiseren",
        "Laden van spelbestanden",
        "Laden van karakters",
        "Kalibreren van besturing",
        "Bijna klaar",
        "Laden voltooid!",
      ],
    },
    ...
    tr: {
      ...
      texts: [
        "Başlatılıyor",
        "Oyun kaynakları yükleniyor",
        "Karakterler yükleniyor",
        "Kontroller kalibre ediliyor",
        "Neredeyse bitti",
        "Yükleme tamamlandı!",
      ],
    },
    hi: {
      ...
      texts: [
        "प्रारंभ हो रहा है",
        "गेम संसाधन लोड हो रहे हैं",
        "किरदार लोड हो रहे हैं",
        "नियंत्रण कैलिब्रेट हो रहे हैं",
        "लगभग वहाँ",
        "लोडिंग पूर्ण!",
      ],
    },
    ...
    id: {
      ...
      texts: [
        "Inisialisasi",
        "Memuat aset permainan",
        "Memuat karakter",
        "Mengkalibrasi kontrol",
        "Hampir selesai",
        "Pemuatan Selesai!",
      ],
    },
    ur: {
      ...
      texts: [
        "شروع کر رہا ہے",
        "گیم اثاثے لوڈ ہو رہے ہیں",
        "کرداروں کو لوڈ کر رہا ہے",
        "کنٹرول کو کیلیبریٹ کر رہا ہے",
        "تقریباً مکمل",
        "لوڈنگ مکمل!",
      ],
    },
  ...
  function parseSvgContent() {
    ...
    outlineSvg.setAttribute("width", "15.625em");
    outlineSvg.setAttribute("height", "15.625em");
    ...
    fillSvg.setAttribute("width", "15.625em");
    fillSvg.setAttribute("height", "15.625em");
  ...
  function createStyles() {
    ...
    style.textContent = `
      :root {
        ...
        /* Base size for em calculations (default: 16px) */
      }
      ...
      .shine-behind {
        ...
        width: 15.625em;
        height: 15.625em;
        background: radial-gradient(circle, var(--progress-color) 0%, transparent 60%);
        border-radius: 50%;
        opacity: 0;
      ...
      @keyframes twinkle {
        ...
        50% {
          ...
          transform: translateY(-0.625em) scale(1);
        }
        100% {
          opacity: 0;
      ...
      #game-icon-container {
        ...
        justify-content: center;
        align-items: center;
        box-shadow: 0 0.625em 1.5625em rgba(0, 0, 0, 0.25);
      }
      ...
      #game-title {
        ...
        animation: fadeInTitle 0.7s forwards ease-out;
      }
      ...
      #loading-bar-background {
        ...
        overflow: hidden;
        box-shadow: 0 0.0625em 0.3125em rgba(0, 0, 0, 0.1) inset;
      }
      ...
      #loading-bar-progress {
        ...
        background-color: var(--progress-color);
        border-radius: 0.25em;
        box-shadow: 0 0 0.625em var(--progress-color);
        position: relative;
      }
  ...
  function createHtmlStructure() {
    ...
    if (outlineSvg && fillSvg) {
      ...
      logoOutlineSvg.setAttribute("width", "15.625em");
      logoOutlineSvg.setAttribute("height", "15.625em");
      ...
      circlePath.setAttribute("stroke-width", "0.125em");
      ...
      logoFillSvg.setAttribute("width", "15.625em");
      logoFillSvg.setAttribute("height", "15.625em");
  ...
  function createStarParticles() {
    ...
    for (let i = 0; i < numberOfStars; i++) {
      ...
      // Size in em units (between 0.0625em and 0.1875em)
      const size = Math.random() * 0.125 + 0.0625;
  ...
  function updateWaveFill() {
    ...
    for (let i = 0; i <= segments; i++) {
      ...
      // Convert px to em for wave height (20px → 1.25em)
      const waveHeightInEm = config.animation.waveHeight / 16;

      // Wave amplitude decreases as progress increases
      const amplitude = waveHeightInEm * (1 - progress / 100);
      const y =
        fillPercentage +
  ...
  function createParticles(count) {
    ...
    const spawnRadius = 30; // This value will be used in px for positioning
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Size in em (between 0.3125em and 0.625em)
      const size = Math.random() * 0.3125 + 0.3125;
  ...
  function updateProgress(newProgress) {
    ...
    if (!loadingBarProgress) return;
    loadingBarProgress.style.width = `${progress}%`;

    // Add a pulsing glow effect when progress is near completion
    if (progress > 90) {
      loadingBarProgress.style.boxShadow = `0 0 0.625em 0.125em var(--progress-color)`;
    }
    ...
    const shadowSize = (progress / 100) * 0.9375; // 15px → 0.9375em

    logoContainer.style.transform = `scale(${scaleValue})`;
    logoContainer.style.filter = `drop-shadow(0px ${shadowSize}em ${shadowSize}em rgba(0, 0, 0, 0.5))`;
  ...
  function createRipple(centerX, centerY, color, isFinal) {
    ...
    // Calculate max size in pixels for the animation
    const maxSize =
      Math.sqrt(
    ...
    const rippleAnimation = () => {
      ...
      // Use pixel values for the ripple as it needs to cover the screen
  ...
  function inversedRipple(ripple, startSize) {
    ...
    const rippleAnimation = () => {
      ...
      // Use pixel values for the inverse ripple
      ripple.style.width = `${size}px`;
      ripple.style.height = `${size}px`;
  ...
  function callLoadingEndCallback() {
    if (loadingEndCallback && typeof loadingEndCallback === "function") {
      loadingEndCallback();
      loadingEndCallback = null; // Clear the callback after calling it
    }
  }
  ...
  return {
    ...
    init: function (options = {}) {
      ...
      const baseScale = window.innerHeight / 1080;
     ...
     * @returns {Object} - The DedraLoader instance for chaining
     */
    onLoadingSafelyEnded: function (callback) {
      loadingEndCallback = callback;
      return this;
    },
